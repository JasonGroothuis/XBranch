...
int main(int argc, char** argv)
{
	...
	if (!hGetProcIDDLL) {
		printf("could not load the nvcuda dynamic library\n");
		return EXIT_FAILURE;
    ...
    else 
    {
        ...
        {
            printf("Error: Failed to malloc for platform IDs\n");    
            return EXIT_FAILURE;        
        }
    ...
    for(cl_uint i = 0; i < num_platforms; ++i)
    {
        ...
        if(err != CL_SUCCESS)
        {
              printf("Error: Failed to get platform infor for platform %d\n",i);
              return EXIT_FAILURE;
        }
    }
